...
app = FastAPI(
    ...
    version="1.0.0",
    description=(
        "FastAPI service for resume parsing, AI analysis, "
...
async def analyze_resumes(
    ...
    try:
        # ------------------------------------------------------------
        # 1. Save uploaded resumes temporarily and extract documents
        # ------------------------------------------------------------
        for upload_index, upload in enumerate(files, 1):
            ...
            if suffix not in {".pdf", ".docx", ".txt"}:
                raise HTTPException(
                    status_code=400,
                    detail=(
                        f"Unsupported file format: {upload.filename}. "
            ...
            try:
                resume_docs = load_resume(temp_path)

                candidate_name = (
                    f"Candidate {upload_index}: "
        ...
        if not documents:
            raise HTTPException(
                status_code=400,
                detail="No valid resume content could be extracted.",
            )

        # ------------------------------------------------------------
        # 2. Update persistent Chroma store
        #
        # IMPORTANT:
        # create_vector_store() no longer deletes the Chroma directory.
        # It removes the old collection documents and inserts the new ones.
        # ------------------------------------------------------------
        try:
            create_vector_store(
                documents,
            ...
            raise HTTPException(
                status_code=503,
                detail=f"Vector store creation failed: {exc}",
            ) from exc

        # ------------------------------------------------------------
        # 3. Analyze resumes using the LLM
        # ------------------------------------------------------------
        try:
        ...
        # ------------------------------------------------------------
        # 5. Always delete temporary uploaded files
        # ------------------------------------------------------------
